...
@DisplayName("복사 생성자 + Collections.unmodifiableList 반환 시 테스트")
public class CopyConstructorTest {
    ...
    void init() {
        ...
        copiedList = racingCars.getCopyConstructorCars();
    }
    ...
    class originalListTest {
        ...
        @Test
        @DisplayName("원본 리스트 안의 객체 요소 변경 시 복사한 리스트 안의 객체 요소는 변경되지 않는다.")
        void test2() {
            racingCars.setCarElementName(0, "changeSeongHa");

            Assertions.assertEquals(copiedList.get(0).getName(), originalCar1Name);
            Assertions.assertNotEquals(copiedList.get(0).getName(), "changeSeongHa");
        }
    }
    ...
    class copiedListTest {
        ...
        @Test
        @DisplayName("복사한 리스트 안의 객체 요소가 변경되어도 원본 리스트 안의 객체 요소는 변경되지 않는다.")
        void test6() {
            copiedList.get(0).setName("changeSeongHa");

            Assertions.assertEquals(racingCars.getCarElementName(0), "seongHa");
            Assertions.assertNotEquals(racingCars.getCarElementName(0), "changeSeongHa");
        }
    }
}
